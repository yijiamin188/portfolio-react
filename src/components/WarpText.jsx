...
const buildTextCanvas = ({ container, width, height, dpr, props }) => {
  ...
  lines.forEach((line, index) => {
    const x = props.textAlign === 'left' ? measureLine(line, letterSpacing) / 2 : width / 2;
...
/**
 * 覆盖层模式：children 是原版 HTML 标题（排版 100% 不变），
 * WebGL 画布绝对定位覆盖在上层做玻璃扭曲。
 * WebGL 正常渲染后（is-enhanced）隐藏底层标题文字但保留占位与星号；
 * WebGL 失败（is-failed）则画布不显示，底层原版标题原样可见。
 */
const WarpText = ({
  ...
  return (
    <div
      ref={containerRef}
      className={`warp-wrap ${enhanced ? 'is-enhanced' : ''} ${failed ? 'is-failed' : ''} ${className}`.trim()}
      style={style}
    >
      {children}
    </div>
